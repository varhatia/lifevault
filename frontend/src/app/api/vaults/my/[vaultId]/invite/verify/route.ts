...
/**
 * @route   GET /api/vaults/my/[vaultId]/invite/verify
...
export async function GET(
  ...
  try {
    ...
    const member = await prisma.myVaultMember.findFirst({
      where: {
        myVaultId: vaultId,
        inviteToken: token,
        isActive: true,
    ...
    return NextResponse.json({
      success: true,
      vaultName: member.myVault.name,
      inviterName: member.myVault.owner.fullName || member.myVault.owner.email,
      role: member.role,
