...
/**
 * @route   POST /api/vaults/my/[vaultId]/invite/accept
...
export async function POST(
  ...
  try {
    ...
    const memberByToken = await prisma.myVaultMember.findFirst({
      where: {
        myVaultId: vaultId,
        inviteToken: token,
        isActive: true,
    ...
    try {
      ...
      const updatedMember = await prisma.myVaultMember.update({
        where: { id: member.id },
        data: {
